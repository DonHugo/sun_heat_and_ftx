...
class SimpleRateLimiter:
    ...
    def __init__(
        ...
        logger.info(
            f"Rate limiter initialized: {default_limit} req/{window_seconds}s, burst: {burst_limit} req/{burst_window}s"
        )
    ...
    def _cleanup_old_requests(self, ip: str, current_time: float):
        ...
        self.requests[ip] = [
            (ts, endpoint) for ts, endpoint in self.requests[ip] if ts > cutoff
        ]
    ...
    def is_allowed(self, ip: str, endpoint: str = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        window_requests = [
            ts for ts, _ in self.requests.get(ip, []) if ts > window_cutoff
        ]
        ...
        burst_requests = [
            ts for ts, _ in self.requests.get(ip, []) if ts > burst_cutoff
        ]
        ...
        if burst_count >= self.burst_limit:
            ...
            logger.warning(
                f"Rate limit exceeded (burst) for {ip}: {burst_count}/{self.burst_limit} in {self.burst_window}s (violation #{self.violations[ip]})"
            )
            return False, info
        ...
        if window_count >= limit:
            ...
            logger.warning(
                f"Rate limit exceeded (window) for {ip}: {window_count}/{limit} in {window}s (violation #{self.violations[ip]})"
            )
            return False, info
...
def check_rate_limit():
    ...
    if not allowed:
        ...
        response_dict, status = create_error_response(
            error_code=APIErrorCode.RATE_LIMIT_EXCEEDED,
            details=f"Retry after {info.get('retry_after', 60)}s (limit {info['limit']}/{info['window']}s, type {info.get('limit_type', 'unknown')})",
            http_status=429,
        )
        error_response = make_response(jsonify(response_dict), status)
