...
class SolarHeatingSystem:
    ...
    def _calculate_rate_of_change(self):
        ...
        try:
            ...
            if current_energy is None or (
                isinstance(current_energy, float) and math.isnan(current_energy)
            ):
                current_energy = 0
            if current_temp is None or (
                isinstance(current_temp, float) and math.isnan(current_temp)
            ):
                logger.warning(
                    f"Invalid average_temperature: {current_temp}, skipping rate calculation"
                )
            ...
            for i in range(len(self.rate_data["temp_values"])):
                ...
                if not (isinstance(temp_val, float) and math.isnan(temp_val)) and not (
                    isinstance(energy_val, float) and math.isnan(energy_val)
                ):
            ...
            if len(valid_data_indices) < len(self.rate_data["timestamps"]):
                logger.warning(
                    f"Removing {len(self.rate_data['timestamps']) - len(valid_data_indices)} invalid rate samples (nan values)"
                )
                self.rate_data["timestamps"] = [
                    self.rate_data["timestamps"][i] for i in valid_data_indices
                ]
                self.rate_data["energy_values"] = [
                    self.rate_data["energy_values"][i] for i in valid_data_indices
                ]
                self.rate_data["temp_values"] = [
                    self.rate_data["temp_values"][i] for i in valid_data_indices
                ]
            ...
            valid_indices = [
                i
                for i, t in enumerate(self.rate_data["timestamps"])
                if t >= window_start
            ]
            ...
            time_diff_hours = (
                self.rate_data["timestamps"][newest_idx]
                - self.rate_data["timestamps"][oldest_idx]
            ) / 3600
            ...
            # Calculate raw rates
            energy_diff = (
                self.rate_data["energy_values"][newest_idx]
                - self.rate_data["energy_values"][oldest_idx]
            )
            temp_diff = (
                self.rate_data["temp_values"][newest_idx]
                - self.rate_data["temp_values"][oldest_idx]
            )
            ...
            if self.rate_config["smoothing"] == "raw":
                ...
                if len(valid_indices) >= 3:
                    ...
                    for i in range(len(valid_indices) - 1):
                        if i + 1 < len(valid_indices):
                            ...
                            e_rate = (
                                self.rate_data["energy_values"][t2]
                                - self.rate_data["energy_values"][t1]
                            ) / (
                                (
                                    self.rate_data["timestamps"][t2]
                                    - self.rate_data["timestamps"][t1]
                                )
                                / 3600
                            )
                            t_rate = (
                                self.rate_data["temp_values"][t2]
                                - self.rate_data["temp_values"][t1]
                            ) / (
                                (
                                    self.rate_data["timestamps"][t2]
                                    - self.rate_data["timestamps"][t1]
                    ...
                    if recent_energy_rates and recent_temp_rates:
                        energy_rate = sum(recent_energy_rates) / len(
                            recent_energy_rates
                        )
                ...
                if hasattr(self, "_last_smoothed_energy_rate"):
                    ...
                    energy_rate = (
                        alpha * raw_energy_rate
                        + (1 - alpha) * self._last_smoothed_energy_rate
                    )
                    temp_rate = (
                        alpha * raw_temp_rate
                        + (1 - alpha) * self._last_smoothed_temp_rate
                    )
                else:
                    energy_rate = raw_energy_rate
                    temp_rate = raw_temp_rate
            ...
            logger.debug(
                f"Rate calculation: Energy: {energy_rate:.3f} kW, Temp: {temp_rate:.2f} °C/h"
            )
            ...
            for key, value in new_config.items():
                if key in self.rate_config:
                    ...
                    if key == "smoothing":
                        ...
                        logger.info(
                            f"Rate smoothing buffers reset for new method: {value}"
                        )
    ...
    def _save_system_state(self):
        ...
        try:
            ...
            state_data = {
                ...
                "heating_cycles_count": self.system_state.get(
                    "heating_cycles_count", 0
                ),
                "total_heating_time": self.system_state.get("total_heating_time", 0.0),
                "total_heating_time_lifetime": self.system_state.get(
                    "total_heating_time_lifetime", 0.0
                ),
                # Energy collection data (daily counters)
                "energy_collected_today": self.system_state.get(
                    "energy_collected_today", 0.0
                ),
                "solar_energy_today": self.system_state.get("solar_energy_today", 0.0),
                "cartridge_energy_today": self.system_state.get(
                    "cartridge_energy_today", 0.0
                ),
                "pellet_energy_today": self.system_state.get(
                    "pellet_energy_today", 0.0
                ),
                # Reset tracking
                "last_midnight_reset_date": self.system_state.get(
            ...
            logger.info(
                f"✅ System state saved: pump_runtime={state_data['pump_runtime_hours']:.2f}h, cycles={state_data['heating_cycles_count']}, daily_time={state_data['total_heating_time']:.2f}h, energy_today={state_data['energy_collected_today']:.2f}kWh"
            )
    ...
    def _load_system_state(self):
        ...
        try:
            ...
            if os.path.exists(state_file):
                ...
                self.system_state["pump_runtime_hours"] = saved_state.get(
                    "pump_runtime_hours", 0.0
                )
                self.system_state["heating_cycles_count"] = saved_state.get(
                    "heating_cycles_count", 0
                )
                self.system_state["total_heating_time"] = saved_state.get(
                    "total_heating_time", 0.0
                )
                self.system_state["total_heating_time_lifetime"] = saved_state.get(
                    "total_heating_time_lifetime", 0.0
                )

                # Restore energy collection data (daily counters)
                self.system_state["energy_collected_today"] = saved_state.get(
                    "energy_collected_today", 0.0
                )
                self.system_state["solar_energy_today"] = saved_state.get(
                    "solar_energy_today", 0.0
                )
                self.system_state["cartridge_energy_today"] = saved_state.get(
                    "cartridge_energy_today", 0.0
                )
                self.system_state["pellet_energy_today"] = saved_state.get(
                    "pellet_energy_today", 0.0
                )

                # Restore reset tracking
                self.system_state["last_midnight_reset_date"] = saved_state.get(
                    "last_midnight_reset_date", ""
                )
                ...
                logger.info(
                    f"✅ System state loaded: pump_runtime={self.system_state['pump_runtime_hours']:.2f}h, cycles={self.system_state['heating_cycles_count']}, daily_time={self.system_state['total_heating_time']:.2f}h, energy_today={self.system_state['energy_collected_today']:.2f}kWh (last saved: {last_save_date})"
                )
            else:
                logger.info(
                    "ℹ️  No saved system state found - starting with fresh counters"
                )
    ...
    def _is_midnight_reset_needed(self):
        ...
        if (
            current_seconds >= 23 * 3600 + 59 * 60 + 50
        ):  # After 23:59:50 (10 seconds before midnight)
        ...
        if time_diff <= 10:
            ...
            if last_reset_date != today:
                logger.info(
                    f"🕛 Midnight reset check: current_time={current_time}, time_diff={time_diff}s, last_reset={last_reset_date}, today={today}"
                )
                return True
    ...
    async def start(self):
        ...
        try:
            ...
            if not HARDWARE_AVAILABLE:
                logger.warning(
                    "Hardware libraries not available - forcing simulation mode"
                )
            ...
            self.robust_sensor_reader = RobustSensorReader(
                self.hardware, max_retries=3, initial_backoff_ms=50
            )
            ...
            for attempt in range(initial_mqtt_retries):
                if self.mqtt.connect():
                    ...
                    if attempt < initial_mqtt_retries - 1:
                        logger.warning(
                            f"MQTT connection failed on attempt {attempt + 1}/{initial_mqtt_retries}, retrying in {initial_retry_delay} seconds..."
                        )
                        await asyncio.sleep(initial_retry_delay)
                    else:
                        logger.warning(
                            f"Initial MQTT connection failed after {initial_mqtt_retries} attempts - will continue retrying in background"
                        )
            ...
            if config.taskmaster_enabled:
                try:
                    ...
                    logger.error(
                        f"Failed to initialize TaskMaster AI service: {str(e)}"
                    )
            else:
            ...
            if API_SERVER_AVAILABLE:
                try:
                    ...
                    self.api_thread = threading.Thread(
                        target=run_api, daemon=True, name="APIServer"
                    )
                    self.api_thread.start()
                    logger.info(
                        "✅ API server started on port 5001 with input validation"
                    )
                except Exception as e:
            ...
            while self.running:
                ...
                if (
                    state_save_counter >= 60
                ):  # Assuming temperature_update_interval is 5 seconds
    ...
    async def _publish_hass_discovery(self):
        ...
        try:
            ...
            logger.info(
                f"Total sensors configured: {len(sensors)} (regular: {len(regular_sensors)}, binary: {len(binary_sensors)})"
            )
            logger.info(
                f"Publishing discovery for {len(regular_sensors)} regular sensors..."
            )
            ...
            for sensor in regular_sensors:
                ...
                topic = (
                    f"homeassistant/sensor/solar_heating_{sensor['entity_id']}/config"
                )
                logger.info(
                    f"Attempting to publish discovery for {sensor['name']} to {topic}"
                )
                success = self.mqtt.publish(topic, config, retain=True)
                logger.info(f"MQTT publish result for {sensor['name']}: {success}")
                if success:
                    logger.info(
                        f"Published HA discovery for {sensor['name']} to {topic}"
                    )
                    discovery_count += 1
                else:
                    logger.error(
                        f"Failed to publish HA discovery for {sensor['name']} to {topic}"
                    )

            logger.info(
                f"Published discovery for {discovery_count} regular sensors successfully"
            )
            ...
            for binary_sensor in overheating_binary_sensors:
                ...
                if success:
                    logger.info(
                        f"Published binary sensor discovery: {binary_sensor['name']}"
                    )
                else:
                    logger.error(
                        f"Failed to publish binary sensor discovery: {binary_sensor['name']}"
                    )

            # Publish binary sensor discovery configurations
            logger.info(
                f"Publishing discovery for {len(binary_sensors)} binary sensors..."
            )
            ...
            for sensor in binary_sensors:
                ...
                logger.info(
                    f"Attempting to publish binary sensor discovery for {sensor['name']} to {topic}"
                )
                success = self.mqtt.publish(topic, config, retain=True)
                logger.info(
                    f"MQTT publish result for binary sensor {sensor['name']}: {success}"
                )
                if success:
                    logger.info(
                        f"Published HA discovery for binary sensor {sensor['name']} to {topic}"
                    )
                    binary_discovery_count += 1
                else:
                    logger.error(
                        f"Failed to publish HA discovery for binary sensor {sensor['name']} to {topic}"
                    )

            logger.info(
                f"Published discovery for {binary_discovery_count} binary sensors successfully"
            )
            ...
            for switch in switches:
                ...
                topic = (
                    f"homeassistant/switch/solar_heating_{switch['entity_id']}/config"
                )
            ...
            for number in numbers:
                ...
                topic = (
                    f"homeassistant/number/solar_heating_{number['entity_id']}/config"
                )
                self.mqtt.publish(topic, config, retain=True)
        ...
        try:
            ...
            for sensor_id in range(8):
                ...
                if status == self.sensor_health_monitor.STATUS_HEALTHY:
                    logger.debug(
                        f"{sensor_name}: {value_to_use}°C (attempts: {attempt_count})"
                    )
                elif status == self.sensor_health_monitor.STATUS_DEGRADED:
                    logger.warning(
                        f"{sensor_name}: {value_to_use}°C (DEGRADED - using last known good)"
                    )
                    ...
                    if self.sensor_health_monitor.should_alert(
                        sensor_name, alert_threshold=5
                    ):
                        if self.mqtt and self.mqtt.is_connected():
                            alert_topic = (
                                f"{config.mqtt_root_topic}/sensor_alerts/{sensor_name}"
                            )
                            alert_payload = {
                                "sensor_id": sensor_name,
                                "status": "failed",
                                "consecutive_errors": self.sensor_health_monitor._error_counts.get(
                                    sensor_name, 0
                                ),
                                "timestamp": time.time(),
                            }
            ...
            logger.info(
                f"RTD sensors read: {[self.temperatures.get(f'rtd_sensor_{i}', 'None') for i in range(8)]}"
            )

            # Issue #50 Fix: Read ALL MegaBAS sensors (1-8, 8 total sensors) with retry logic
            megabas_temps, megabas_attempts = (
                self.robust_sensor_reader.read_all_megabas_sensors()
            )
            ...
            logger.info(
                f"MegaBAS sensors read: {[self.temperatures.get(f'megabas_sensor_{i}', 'None') for i in range(1, 9)]}"
            )
            ...
            if failed_sensors:
                logger.error(
                    f"Failed sensors ({len(failed_sensors)}): {', '.join(failed_sensors)}"
                )
            if degraded_sensors:
                logger.warning(
                    f"Degraded sensors ({len(degraded_sensors)}): {', '.join(degraded_sensors)}"
                )
            ...
            self.temperatures["outdoor_air_temp"] = self.temperatures.get(
                "megabas_sensor_1"
            )  # Outdoor air intake
            self.temperatures["exhaust_air_temp"] = self.temperatures.get(
                "megabas_sensor_2"
            )  # Air leaving heat exchanger
            self.temperatures["supply_air_temp"] = self.temperatures.get(
                "megabas_sensor_3"
            )  # Air entering heat exchanger
            self.temperatures["return_air_temp"] = self.temperatures.get(
                "megabas_sensor_4"
            )  # Air returning from heat exchanger
            ...
            self.temperatures["solar_collector"] = self.temperatures.get(
                "megabas_sensor_6"
            )  # T1 - Solar panel outlet
            self.temperatures["storage_tank"] = self.temperatures.get(
                "megabas_sensor_7"
            )  # T2 - Main storage tank
            self.temperatures["return_line_temp"] = self.temperatures.get(
                "megabas_sensor_8"
            )  # T3 - Return line to solar collector

            # Also maintain the _temp versions for backward compatibility
            self.temperatures["solar_collector_temp"] = self.temperatures.get(
                "megabas_sensor_6"
            )  # T1 - Solar panel outlet
            self.temperatures["storage_tank_temp"] = self.temperatures.get(
                "megabas_sensor_7"
            )  # T2 - Main storage tank
            ...
            self.temperatures["water_heater_bottom"] = self.temperatures.get(
                "rtd_sensor_0"
            )  # RTD sensor 0 - 0cm from bottom (coldest)
            self.temperatures["water_heater_20cm"] = self.temperatures.get(
                "rtd_sensor_1"
            )  # RTD sensor 1 - 20cm from bottom
            self.temperatures["water_heater_40cm"] = self.temperatures.get(
                "rtd_sensor_2"
            )  # RTD sensor 2 - 40cm from bottom
            self.temperatures["water_heater_60cm"] = self.temperatures.get(
                "rtd_sensor_3"
            )  # RTD sensor 3 - 60cm from bottom
            self.temperatures["water_heater_80cm"] = self.temperatures.get(
                "rtd_sensor_4"
            )  # RTD sensor 4 - 80cm from bottom
            self.temperatures["water_heater_100cm"] = self.temperatures.get(
                "rtd_sensor_5"
            )  # RTD sensor 5 - 100cm from bottom
            self.temperatures["water_heater_120cm"] = self.temperatures.get(
                "rtd_sensor_6"
            )  # RTD sensor 6 - 120cm from bottom
            self.temperatures["water_heater_140cm"] = self.temperatures.get(
                "rtd_sensor_7"
            )  # RTD sensor 7 - 140cm from bottom (hottest)
            logger.info("Water heater RTD sensors mapped successfully")

            # Debug logging for water heater sensors
            logger.debug(
                f"Water heater sensors: bottom={self.temperatures['water_heater_bottom']}, 20cm={self.temperatures['water_heater_20cm']}, 40cm={self.temperatures['water_heater_40cm']}, 60cm={self.temperatures['water_heater_60cm']}, 80cm={self.temperatures['water_heater_80cm']}, 100cm={self.temperatures['water_heater_100cm']}, 120cm={self.temperatures['water_heater_120cm']}, 140cm={self.temperatures['water_heater_140cm']}"
            )
            ...
            self.temperatures["heat_exchanger_in"] = self.temperatures.get(
                "megabas_sensor_1"
            )
            self.temperatures["heat_exchanger_out"] = self.temperatures.get(
                "megabas_sensor_2"
            )
            ...
            water_heater_140cm = self.temperatures.get(
                "water_heater_140cm", 0
            )  # Top sensor
            water_heater_bottom = self.temperatures.get(
                "water_heater_bottom", 0
            )  # Bottom sensor (0cm)
            if water_heater_140cm and water_heater_bottom:
                stratification_quality = round(
                    (water_heater_140cm - water_heater_bottom) / 140, 2
                )  # 140cm height
                gradient_per_cm = round(
                    (water_heater_140cm - water_heater_bottom) / 140, 3
                )
                self.temperatures["water_heater_stratification"] = (
                    stratification_quality
                )
                self.temperatures["water_heater_gradient_cm"] = gradient_per_cm
                logger.debug(
                    f"Stratification quality: {stratification_quality}, Gradient: {gradient_per_cm}°C/cm"
                )
            ...
            if config.taskmaster_enabled:
                try:
                    ...
                    taskmaster_temp_data = {
                        "solar_collector_temp": self.temperatures.get(
                            "solar_collector_temp", 0
                        ),
                        "storage_tank_temp": self.temperatures.get(
                            "storage_tank_temp", 0
                        ),
                        "return_line_temp": self.temperatures.get(
                            "return_line_temp", 0
                        ),
                        "water_heater_bottom": self.temperatures.get(
                            "water_heater_bottom", 0
                        ),
                        "water_heater_100cm": self.temperatures.get(
                            "water_heater_100cm", 0
                        ),
                        "outdoor_air": self.temperatures.get("outdoor_air_temp", 0),
                        "heat_exchanger_in": self.temperatures.get(
                            "heat_exchanger_in", 0
                        ),
                        "heat_exchanger_out": self.temperatures.get(
                            "heat_exchanger_out", 0
                        ),
                    }

                    await taskmaster_service.process_temperature_data(
                        taskmaster_temp_data
                    )
                    logger.debug("Temperature data processed by TaskMaster AI service")
                except Exception as e:
                    logger.error(
                        f"Error processing temperature data with TaskMaster AI: {str(e)}"
                    )
            ...
            if solar_collector_temp > max_safe_temp:
                overheating_risk = round(
                    (
                        (solar_collector_temp - max_safe_temp)
                        / (max_risk_temp - max_safe_temp)
                    )
                    * 100,
                    1,
                )
                ...
                logger.warning(
                    f"Overheating risk: {overheating_risk}% (collector: {solar_collector_temp}°C)"
                )
            else:
            ...
            is_heating = (
                self.system_state.get("primary_pump", False)
                and self.system_state.get("mode") == "heating"
            )
            ...
            self.temperatures["heating_cycles_count"] = self.system_state.get(
                "heating_cycles_count", 0
            )
            self.temperatures["total_heating_time_lifetime"] = self.system_state.get(
                "total_heating_time_lifetime", 0.0
            )
            ...
            if self.system_state.get("primary_pump", False) and self.system_state.get(
                "last_pump_start"
            ):
            ...
            self.temperatures["pump_runtime_hours_realtime"] = round(
                real_time_runtime, 2
            )

            # Calculate average heating duration
            if self.system_state.get("heating_cycles_count", 0) > 0:
                avg_heating_duration = round(
                    self.system_state.get("total_heating_time", 0)
                    / self.system_state.get("heating_cycles_count", 1),
                    2,
                )
            ...
            logger.debug(
                f"Operational metrics: pump_runtime={self.temperatures['pump_runtime_hours']}h, cycles={self.temperatures['heating_cycles_count']}, avg_duration={self.temperatures['average_heating_duration']}h, lifetime={self.temperatures['total_heating_time_lifetime']}h"
            )
            logger.info(
                f"System state operational metrics: pump_runtime={self.system_state.get('pump_runtime_hours', 0)}h, cycles={self.system_state.get('heating_cycles_count', 0)}, daily_time={self.system_state.get('total_heating_time', 0)}h, lifetime={self.system_state.get('total_heating_time_lifetime', 0)}h, last_start={self.system_state.get('last_pump_start', 'None')}"
            )
            ...
            dT = (
                solar_collector - storage_tank
                if solar_collector and storage_tank
                else 0
            )
            ...
            stored_energy_kwh = [
                ...
                round(
                    sum(stored_energy[5:]) / 3600, 2
                ),  # Top 4 sensors (RTD 5-7 + MegaBAS 5)
                round(
                    sum(
                        [
                            self.temperatures.get(f"rtd_sensor_{i}", 0) or 0
                            for i in range(8)
                        ]
                    )
                    / 8,
            ...
            logger.info(
                f"Stored Energy - Total: {stored_energy_kwh[0]} kWh, Top: {stored_energy_kwh[2]} kWh, Bottom: {stored_energy_kwh[1]} kWh, Avg Temp: {stored_energy_kwh[3]}°C"
            )
            logger.debug(
                f"RTD sensor values: {[self.temperatures.get(f'rtd_sensor_{i}', 0) for i in range(8)]}"
            )
            logger.debug(
                f"MegaBAS sensor 5: {self.temperatures.get('megabas_sensor_5', 0)}"
            )
            logger.info("Stored energy debug logging completed")

            # Log expected energy range for 360L tank (4°C to 90°C = max ~36 kWh)
            max_expected_energy = round(
                (tank_volume_kg * specific_heat_capacity * (90 - zero_value)) / 3600, 2
            )
            if stored_energy_kwh[0] > max_expected_energy * 1.1:  # Allow 10% tolerance
                logger.warning(
                    f"⚠️  Stored energy ({stored_energy_kwh[0]} kWh) exceeds expected maximum ({max_expected_energy} kWh) for {tank_volume_liters}L tank!"
                )
            else:
                logger.info(
                    f"✅ Energy calculation within expected range: {stored_energy_kwh[0]} kWh (max expected: {max_expected_energy} kWh)"
                )
            ...
            if (
                current_time - self.system_state.get("last_hour_reset", 0) >= 3600
            ):  # 1 hour
            ...
            if self._is_midnight_reset_needed():
                ...
                self.system_state["last_midnight_reset_date"] = (
                    datetime.now().date().isoformat()
                )
                self.system_state["last_day_reset"] = current_time

                logger.info(
                    "🕛 MIDNIGHT RESET COMPLETED: Daily counters and operational metrics reset"
                )
                logger.info(
                    f"  📊 Energy: {self.system_state['energy_collected_today']:.2f} kWh, Solar: {self.system_state['solar_energy_today']:.2f} kWh, Cartridge: {self.system_state['cartridge_energy_today']:.2f} kWh, Pellet: {self.system_state['pellet_energy_today']:.2f} kWh"
                )
                logger.info(
                    f"  ⚙️  Pump Runtime: {self.system_state['pump_runtime_hours']:.2f}h, Heating Cycles: {self.system_state['heating_cycles_count']}, Daily Time: {self.system_state['total_heating_time']:.2f}h, Lifetime: {self.system_state['total_heating_time_lifetime']:.2f}h"
                )
            ...
            if self.system_state.get("last_energy_calculation"):
                ...
                if time_diff > 0:
                    ...
                    energy_diff = current_stored_energy - self.system_state.get(
                        "last_stored_energy", current_stored_energy
                    )
                    if energy_diff > 0:  # Only count positive energy gains
                        energy_rate_per_hour = (
                            energy_diff / time_diff
                        ...
                        solar_active = (
                            self.system_state.get("primary_pump", False)
                            and self.temperatures.get("solar_collector_temp", 0)
                        ...
                        if solar_active:
                            ...
                            solar_dt = self.temperatures.get(
                                "solar_collector_temp", 0
                            ) - self.temperatures.get("storage_tank_temp", 0)
                            # Rough estimate: higher dT = higher contribution
                            source_contributions["solar"] = min(
                                1.0, max(0.1, solar_dt / 20.0)
                            )  # 0.1 to 1.0 based on dT
                            logger.info(
                                f"Solar heating detected: dT={solar_dt:.1f}°C, contribution={source_contributions['solar']:.2f}"
                            )

                        # Check cartridge heater (relay state)
                        cartridge_active = self.system_state.get(
                            "cartridge_heater", False
                        )
                        if cartridge_active:
                            active_heat_sources.append("cartridge")
                            # Cartridge heater typically provides consistent heating
                            source_contributions["cartridge"] = (
                                0.8  # Assume 80% of energy when active
                            )
                            logger.info(
                                f"Cartridge heater detected: ON, contribution={source_contributions['cartridge']:.2f}"
                            )
                        ...
                        if pellet_active:
                            ...
                            source_contributions["pellet"] = (
                                1.0  # Assume 100% when no other source
                            )
                            logger.info(
                                f"Pellet furnace detected: assumed active, contribution={source_contributions['pellet']:.2f}"
                            )
                        ...
                        if len(active_heat_sources) > 1:
                            total_weight = sum(
                                source_contributions.get(source, 0.5)
                        ...
                        if active_heat_sources:
                            ...
                            for source in active_heat_sources:
                                ...
                                source_contribution = (
                                    hourly_contribution_total * source_weight
                                )

                                if source == "solar":
                                    self.system_state["solar_energy_hour"] += (
                                        source_contribution
                                    )
                                    self.system_state["solar_energy_today"] += (
                                        source_contribution
                                    )
                                elif source == "cartridge":
                                    self.system_state["cartridge_energy_hour"] += (
                                        source_contribution
                                    )
                                    self.system_state["cartridge_energy_today"] += (
                                        source_contribution
                                    )
                                elif source == "pellet":
                                    self.system_state["pellet_energy_hour"] += (
                                        source_contribution
                                    )
                                    self.system_state["pellet_energy_today"] += (
                                        source_contribution
                                    )

                            logger.info(
                                f"Energy collected: {energy_diff:.3f} kWh in {time_diff / 3600:.2f} hours, Rate: {energy_rate_per_hour:.2f} kWh/hour, Sources: {active_heat_sources}"
                            )
            ...
            self.temperatures["energy_collection_rate_kwh_per_hour"] = round(
                energy_rate_per_hour if "energy_rate_per_hour" in locals() else 0.0, 2
            )
            ...
            self.temperatures["energy_collected_today_kwh"] = round(
                ...
                + self.system_state.get("pellet_energy_today", 0.0),
                2,
            )
            self.temperatures["energy_collected_hour_kwh"] = round(
                ...
                + self.system_state.get("pellet_energy_hour", 0.0),
                2,
            )

            # Add heat source specific energy metrics
            self.temperatures["solar_energy_today_kwh"] = round(
                self.system_state.get("solar_energy_today", 0.0), 2
            )
            self.temperatures["cartridge_energy_today_kwh"] = round(
                self.system_state.get("cartridge_energy_today", 0.0), 2
            )
            self.temperatures["pellet_energy_today_kwh"] = round(
                self.system_state.get("pellet_energy_today", 0.0), 2
            )
            self.temperatures["solar_energy_hour_kwh"] = round(
                self.system_state.get("solar_energy_hour", 0.0), 2
            )
            self.temperatures["cartridge_energy_hour_kwh"] = round(
                self.system_state.get("cartridge_energy_hour", 0.0), 2
            )
            self.temperatures["pellet_energy_hour_kwh"] = round(
    ...
    def _calculate_water_heater_safety_metrics(self):
        ...
        try:
            ...
            if len(valid_temps) < 4:
                logger.warning(
                    f"Insufficient valid sensors for safety metrics: {len(valid_temps)}/8"
                )
            ...
            layers_above_threshold = sum(
                1 for t in valid_temps if t >= DANGER_THRESHOLD
            )
            ...
            risk_condition_2 = (
                thermal_mass_top4 >= HIGH_AVG_THRESHOLD
                and layers_above_threshold >= MIN_LAYERS_HIGH
            )
            ...
            if overheating_risk:
                logger.warning(
                    f"⚠️  OVERHEATING RISK DETECTED! Peak: {peak_temp}°C, "
                    f"Top4 avg: {thermal_mass_top4}°C, "
                    f"Layers ≥{DANGER_THRESHOLD}°C: {layers_above_threshold}"
                )
            else:
                logger.debug(
                    f"No overheating risk. Peak: {peak_temp}°C, "
                    f"Top4 avg: {thermal_mass_top4}°C"
                )
    ...
    def _calculate_realtime_energy_sensor(self):
        ...
        try:
            ...
            last_time = self.system_state.get(
                "last_energy_rate_calculation", current_time
            )
            last_stored_energy = self.system_state.get(
                "last_stored_energy_for_rate", current_stored_energy
            )
            last_avg_temp = self.system_state.get(
                "last_avg_temp_for_rate", current_avg_temp
            )
            ...
            energy_temp_ratio = (
                energy_rate_kw / max(current_avg_temp, 1) if current_avg_temp > 0 else 0
            )

            # 2. Energy rate vs total stored energy ratio
            energy_total_ratio = (
                energy_rate_kw / max(current_stored_energy, 1)
                if current_stored_energy > 0
                else 0
            )

            # 3. Temperature change vs average temperature ratio
            temp_change_ratio = (
                temp_rate_per_hour / max(current_avg_temp, 1)
                if current_avg_temp > 0
                else 0
            )
            ...
            efficiency_index = min(
                100, max(0, efficiency_factor * 10)
            )  # Scale to 0-100

            # Store calculated values
            self.temperatures["realtime_energy_rate_kw"] = round(energy_rate_kw, 3)
            self.temperatures["realtime_temp_rate_per_hour"] = round(
                temp_rate_per_hour, 2
            )
            ...
            if energy_rate_kw > 0:  # Positive energy gain (heating)
                ...
                performance_score += min(
                    30, energy_rate_kw * 10
                )  # Bonus for higher rate (max 30)
                performance_score += min(
                    20, efficiency_index / 5
                )  # Bonus for efficiency (max 20)
                performance_score += min(
                    10, 10 - abs(temp_rate_per_hour)
                )  # Bonus for stable temperature (max 10)
            ...
            logger.info(
                f"Real-time energy sensor: {energy_rate_kw:.3f} kW, temp rate: {temp_rate_per_hour:.2f}°C/h, efficiency: {efficiency_index:.1f}%, performance: {performance_score:.1f}"
            )
        ...
        try:
            ...
            if self.system_state.get("primary_pump_manual", False):
                ...
                if self.system_state[
                    "primary_pump"
                ] != self.hardware.simulation_relays.get(1, False):
                    self.hardware.set_relay_state(1, self.system_state["primary_pump"])
                    logger.info(
                        f"Manual override active: Pump forced to {'ON' if self.system_state['primary_pump'] else 'OFF'}"
                    )
            ...
            if solar_collector >= self.control_params["temp_kok"]:
                if self.system_state["primary_pump"]:
                    ...
                    logger.warning(
                        f"Emergency pump stop: Collector temperature {solar_collector}°C >= {self.control_params['temp_kok']}°C"
                    )

            # Emergency stop recovery with hysteresis
            elif self.system_state.get("overheated", False) and solar_collector < (
                self.control_params["temp_kok"]
                - self.control_params["temp_kok_hysteres"]
            ):
                self.system_state["overheated"] = False
                logger.info(
                    f"Emergency stop recovery: Collector temperature {solar_collector}°C < {self.control_params['temp_kok'] - self.control_params['temp_kok_hysteres']}°C (hysteresis recovery)"
                )
            ...
            # Collector cooling logic (medium priority) - from V1 implementation
                ...
                if not self.system_state["primary_pump"]:
                    ...
                    logger.warning(
                        f"Collector cooling activated: Collector temperature {solar_collector}°C >= {self.control_params['kylning_kollektor']}°C. Cycle #{self.system_state['heating_cycles_count']}"
                    )
                    ...
                    if config.taskmaster_enabled:
                        try:
                            await taskmaster_service.process_pump_control(
                                "start",
                                {
                                    "reason": "collector_cooling",
                                    "collector_temp": solar_collector,
                                    "threshold": self.control_params[
                                        "kylning_kollektor"
                                    ],
                                    "cycle_number": self.system_state[
                            ...
                            logger.error(
                                f"Error creating TaskMaster AI collector cooling task: {str(e)}"
                            )

            # Collector cooling stop with hysteresis
            elif self.system_state.get(
                "collector_cooling_active", False
            ) and solar_collector < (
                self.control_params["kylning_kollektor"]
                - self.control_params.get("kylning_kollektor_hysteres", 4.0)
            ):
                if self.system_state["primary_pump"]:
                    self.hardware.set_relay_state(1, False)  # Primary pump relay
                    self.system_state["primary_pump"] = False
                    ...
                    if self.system_state["last_pump_start"]:
                        ...
                        logger.info(
                            f"Collector cooling stopped: Collector temperature {solar_collector}°C < {self.control_params['kylning_kollektor'] - self.control_params.get('kylning_kollektor_hysteres', 4.0)}°C. Cooling cycle runtime: {cycle_runtime:.2f}h"
                        )
            ...
            if (
                ...
                and dT > 10  # Significant temperature difference
                and self.system_state.get("cartridge_heater", False)
            ):  # Cartridge heater is on
                logger.warning(
                    f"Cartridge heater running at night: dT={dT:.1f}°C, collector={solar_collector}°C, tank={storage_tank}°C"
                )
                logger.info(
                    "This explains the unexpected heat source - cartridge heater is active"
                )
            ...
            # Normal control logic
                ...
                if not self.system_state["primary_pump"]:
                    ...
                    self.system_state["heating_cycles_count"] += 1
                    logger.info(
                        f"Primary pump started. dT={dT:.1f}°C >= {self.control_params['dTStart_tank_1']}°C. Cycle #{self.system_state['heating_cycles_count']}"
                    )
                    ...
                    if config.taskmaster_enabled:
                        try:
                            ...
                            logger.error(
                                f"Error creating TaskMaster AI pump control task: {str(e)}"
                            )
                ...
                if self.system_state["primary_pump"]:
                    ...
                    if self.system_state["last_pump_start"]:
                        ...
                        self.system_state["total_heating_time"] += cycle_runtime
                        self.system_state["total_heating_time_lifetime"] += (
                            cycle_runtime  # Add to lifetime total
                        )
                        self.system_state["pump_runtime_hours"] = round(
                            self.system_state["total_heating_time"], 2
                        )
                        logger.info(
                            f"Primary pump stopped. dT={dT:.1f}°C <= {self.control_params['dTStop_tank_1']}°C. Cycle runtime: {cycle_runtime:.2f}h, Daily runtime: {self.system_state['pump_runtime_hours']}h, Lifetime: {self.system_state['total_heating_time_lifetime']:.2f}h"
                        )
                        ...
                        logger.warning(
                            "Primary pump stopped but last_pump_start was None - runtime not calculated"
                        )

            # Keep current state when 4°C < dT < 8°C (hysteresis zone)
            else:
                logger.debug(
                    f"Pump state unchanged. dT={dT:.1f}°C (hysteresis zone: {self.control_params['dTStop_tank_1']}°C < dT < {self.control_params['dTStart_tank_1']}°C)"
                )
    ...
    def _update_system_mode(self):
        ...
        try:
            ...
            if new_mode != old_mode:
                ...
                if new_mode in ["heating", "standby"] and old_mode in [
                    "heating",
                    "standby",
                ]:
                    ...
                    logger.info(
                        f"      ⚙️  Control Thresholds: Start={self.control_params['dTStart_tank_1']}°C, Stop={self.control_params['dTStop_tank_1']}°C"
                    )
                    logger.info(
                        f"      🔧 Pump State: {'ON' if self.system_state.get('primary_pump', False) else 'OFF'}"
                    )
            else:
        ...
        try:
            ...
            if current_mode == "heating":
                reasoning["explanation"] = (
                    f"Heating mode: Pump is ON because dT={dT:.1f}°C >= {self.control_params['dTStart_tank_1']}°C"
                )
            elif current_mode == "standby":
                reasoning["explanation"] = (
                    f"Standby mode: Pump is OFF because dT={dT:.1f}°C < {self.control_params['dTStart_tank_1']}°C"
                )
            elif current_mode == "overheated":
                reasoning["explanation"] = (
                    f"Overheated mode: Emergency stop because collector {solar_collector:.1f}°C >= {self.control_params['temp_kok']}°C (recovery at {self.control_params['temp_kok'] - self.control_params['temp_kok_hysteres']}°C)"
                )
            elif current_mode == "manual":
                reasoning["explanation"] = (
                    "Manual mode: User has manual control override"
                )
            elif current_mode == "collector_cooling":
                reasoning["explanation"] = (
                    f"Collector cooling: Active cooling to prevent overheating at {solar_collector:.1f}°C"
                )
            elif current_mode == "test":
                reasoning["explanation"] = (
                    "Test mode: System running in simulation mode"
                )
            else:
    ...
    def _set_system_mode(self, mode):
        ...
        try:
            ...
            if mode == "auto":
                ...
                self.control_params["dTStart_tank_1"] = (
                    10.0  # Higher threshold for energy saving
                )
                self.control_params["dTStop_tank_1"] = 6.0  # Higher stop threshold
                self.control_params["set_temp_tank_1"] = (
                    55.0  # Lower target temperature
                )
            ...
            logger.info(
                f"      Start Threshold: {self.control_params['dTStart_tank_1']}°C"
            )
            logger.info(
                f"      Stop Threshold: {self.control_params['dTStop_tank_1']}°C"
            )
            logger.info(
                f"      Target Temperature: {self.control_params['set_temp_tank_1']}°C"
            )
            logger.info(
                f"      Manual Control: {self.system_state.get('manual_control', False)}"
            )
    ...
    async def _publish_status(self):
        ...
        try:
            ...
            logger.info(
                f"Starting to publish {total_sensors} sensors to Home Assistant"
            )
            for sensor_name, value in self.temperatures.items():
                if self.mqtt and self.mqtt.is_connected():
                    ...
                    if sensor_name == "heat_exchanger_efficiency":
                        ...
                        sensor_count += 1
                        logger.info(
                            f"Published efficiency sensor: {sensor_name} = {value}"
                        )
                        ...
                        sensor_count += 1
                        logger.info(
                            f"Published system mode sensor: {sensor_name} = {value}"
                        )
                        ...
                        sensor_count += 1
                        logger.info(
                            f"Published heating status sensor: {sensor_name} = {value}"
                        )
                        ...
                        logger.info(
                            f"Published binary sensor {sensor_name} = {binary_message} to {binary_topic}"
                        )
                        ...
                        sensor_count += 1
                        logger.info(
                            f"Published average temperature sensor: {sensor_name} = {value}"
                        )
                    else:
            ...
            if self.mqtt and self.mqtt.is_connected():
                ...
                self._publish_switch_state(
                    "primary_pump", self.system_state["primary_pump"]
                )
                self._publish_switch_state(
                    "primary_pump_manual", self.system_state["primary_pump_manual"]
                )
                self._publish_switch_state(
            ...
            if self.mqtt and self.mqtt.is_connected():
                ...
                self._publish_number_state(
                    "set_temp_tank_1", self.control_params["set_temp_tank_1"]
                )
                self._publish_number_state(
                    "dTStart_tank_1", self.control_params["dTStart_tank_1"]
                )
                self._publish_number_state(
                    "dTStop_tank_1", self.control_params["dTStop_tank_1"]
                )
                self._publish_number_state(
                ...
                self._publish_number_state(
                    "temp_kok_hysteres", self.control_params["temp_kok_hysteres"]
                )
                # Enhanced temperature management
                self._publish_number_state(
                    "morning_peak_target", self.control_params["morning_peak_target"]
                )
                self._publish_number_state(
                    "evening_peak_target", self.control_params["evening_peak_target"]
                )
                self._publish_number_state(
                    "pellet_stove_max_temp",
                    self.control_params["pellet_stove_max_temp"],
                )
                self._publish_number_state(
                    "heat_distribution_temp",
                    self.control_params["heat_distribution_temp"],
                )
                ...
                if config.taskmaster_enabled:
                    try:
                        system_status = {
                            ...
                            "primary_pump": self.system_state.get(
                                "primary_pump", False
                            ),
                            "pump_runtime_hours": self.system_state.get(
                            ...
                            "uptime": time.time()
                            - self.system_state.get("last_update", time.time()),
                            "temperatures": self.temperatures,
                        }
                        await taskmaster_service.process_system_status(system_status)
                    except Exception as e:
                        logger.error(
                            f"Error processing system status with TaskMaster AI: {str(e)}"
                        )
        ...
        try:
            ...
            if self.mqtt and self.mqtt.is_connected():
                for sensor_name in hourly_sensors:
                    ...
                    self.mqtt.publish_raw(topic, message)
                    logger.info(
                        f"Published hourly sensor after reconnection: {sensor_name} = {value} kWh"
                    )
    ...
    async def _publish_hourly_aggregation(self):
        ...
        try:
            ...
            total_sensors = len(self.temperatures)
            logger.info(
                f"Publishing complete {total_sensors} sensors with hourly aggregation"
            )
            ...
            for sensor_name, value in self.temperatures.items():
                if self.mqtt and self.mqtt.is_connected():
                    ...
                    if sensor_name == "heat_exchanger_efficiency":
                        ...
                        message = str(value) if value is not None else "0"
                        logger.info(
                            f"🕐 Published hourly energy sensor: {sensor_name} = {value} kWh (complete hour)"
                        )
                        sensor_count += 1
                    elif sensor_name == "average_temperature":
                        # For average temperature, send the raw number
            ...
            if self.mqtt and self.mqtt.is_connected():
                ...
                self.mqtt.publish_status(status_data)
                logger.info(
                    "System status with hourly aggregation published successfully"
                )
                ...
                self.mqtt.publish_realtime_energy_sensor(self.temperatures)
                logger.info(
                    "Real-time energy sensor with hourly data published successfully"
                )
                ...
                logger.info(
                    f"  📊 Total Energy: {self.temperatures.get('energy_collected_hour_kwh', 0):.3f} kWh"
                )
                logger.info(
                    f"  ☀️  Solar Energy: {self.temperatures.get('solar_energy_hour_kwh', 0):.3f} kWh"
                )
                logger.info(
                    f"  🔥 Cartridge Energy: {self.temperatures.get('cartridge_energy_hour_kwh', 0):.3f} kWh"
                )
                logger.info(
                    f"  🌲 Pellet Energy: {self.temperatures.get('pellet_energy_hour_kwh', 0):.3f} kWh"
                )
        ...
        try:
            if command_type == "switch_command":
                ...
                if self.mqtt and self.mqtt.is_connected():
                    ...
                    logger.info(
                        f"Published mode reasoning: {reasoning['current_mode']} - {reasoning['explanation']}"
                    )
                ...
                if data.get("entity_id") == "solar_heating_system_mode":
                    ...
                    logger.debug(
                        f"Received unexpected MQTT command type: {command_type}"
                    )
        ...
        while self.running:
            try:
                ...
                if is_end_of_hour:
                    ...
                    logger.info(
                        f"🕐 End of hour {current_hour} - published hourly aggregation"
                    )
                    ...
                    sleep_time = min(
                        300, time_until_next_hour - 10
                    )  # 5 minutes or until end of hour
            ...
            try:
                if self.mqtt and self.mqtt.is_connected():
                    ...
                    system_info = {
                        ...
                        "cartridge_heater": self.system_state.get(
                            "cartridge_heater", False
                        ),
                ...
                if self.mqtt and not self.mqtt.is_connected():
                    ...
                    if self.mqtt.connect():
                        ...
                        try:
                            await self._publish_hass_discovery()
                            logger.info(
                                "Home Assistant discovery re-published successfully"
                            )
                        except Exception as e:
                        ...
                        try:
                            await self._publish_hourly_sensors_only()
                            logger.info(
                                "Hourly energy sensors published after reconnection"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error publishing hourly sensors after reconnection: {e}"
                            )

                    else:
                        if consecutive_failures == 1:
                            logger.warning(
                                "MQTT reconnection failed - will continue retrying"
                            )
                        elif consecutive_failures % 10 == 0:  # Log every 10th failure
                            logger.info(
                                f"MQTT connection still unavailable after {consecutive_failures} attempts"
                            )
