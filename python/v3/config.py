...
class SystemConfig(BaseModel):
    ...
    hardware_platform: str = Field(
        default="raspberry_pi_zero_2_w", description="Hardware platform"
    )
    ...
    temperature_update_interval: int = Field(
        default=30, description="Temperature reading interval in seconds"
    )
    temperature_threshold_high: float = Field(
        default=80.0, description="High temperature threshold in Celsius"
    )
    temperature_threshold_low: float = Field(
        default=20.0, description="Low temperature threshold in Celsius"
    )

    # Solar Collector Configuration
    set_temp_tank_1: float = Field(
        default=75.0, description="Target tank temperature (balanced for general use)"
    )
    set_temp_tank_1_hysteres: float = Field(
        default=2.0, description="Tank temperature hysteresis"
    )
    dTStart_tank_1: float = Field(
        default=8.0, description="Temperature difference to start pump"
    )
    dTStop_tank_1: float = Field(
        default=4.0, description="Temperature difference to stop pump"
    )
    kylning_kollektor: float = Field(
        default=90.0,
        description="Collector cooling temperature (maximum safe solar heating)",
    )
    ...
    temp_kok_hysteres: float = Field(
        default=10.0, description="Boiling temperature hysteresis"
    )
    ...
    mqtt_username: Optional[str] = Field(
        default=None,
        description="MQTT username (REQUIRED - set via MQTT_USERNAME env var)",
    )
    mqtt_password: Optional[str] = Field(
        default=None,
        description="MQTT password (REQUIRED - set via MQTT_PASSWORD env var)",
    )
    mqtt_client_id: str = Field(
        default="solar_heating_v3", description="MQTT client ID"
    )
    mqtt_root_topic: str = Field(
        default="solar_heating_v3",
        description="Root MQTT topic for sensor alerts and health",
    )

    # Home Assistant Integration
    hass_enabled: bool = Field(
        default=True, description="Enable Home Assistant integration"
    )
    hass_discovery_prefix: str = Field(
        default="homeassistant", description="HA discovery prefix"
    )

    # TaskMaster AI Configuration
    taskmaster_enabled: bool = Field(
        default=False, description="Enable TaskMaster AI integration"
    )
    taskmaster_api_key: str = Field(default="", description="TaskMaster AI API key")
    taskmaster_base_url: str = Field(
        default="https://api.taskmaster.ai", description="TaskMaster AI base URL"
    )
    ...
    ai_analysis_interval: int = Field(
        default=3600, description="AI analysis interval in seconds"
    )

    # Rate of Change Sensor Configuration
    rate_time_window: str = Field(
        default="medium", description="Rate calculation time window (fast/medium/slow)"
    )
    rate_smoothing: str = Field(
        default="exponential",
        description="Rate smoothing method (raw/simple/exponential)",
    )
    rate_update_interval: int = Field(
        default=30, description="Rate calculation interval in seconds"
    )
    rate_smoothing_alpha: float = Field(
        default=0.3, description="Exponential smoothing factor (0.1-0.9)"
    )

    # Enhanced Temperature Management Configuration
    morning_peak_target: float = Field(
        default=80.0, description="Morning peak temperature target for showers"
    )
    evening_peak_target: float = Field(
        default=75.0, description="Evening peak temperature target for baths"
    )
    pellet_stove_max_temp: float = Field(
        default=55.0, description="Maximum temperature for pellet stove operation"
    )
    heat_distribution_temp: float = Field(
        default=85.0,
        description="Temperature to start continuous pump for heat distribution",
    )
...
def get_calculated_values() -> Dict[str, float]:
    ...
    return {
        "set_temp_tank_1_gräns": config.set_temp_tank_1
        - config.set_temp_tank_1_hysteres,
        "kylning_kollektor_hysteres_gräns": config.kylning_kollektor
        - config.kylning_kollektor_hysteres,
